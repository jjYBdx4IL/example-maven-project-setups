...
if [[  "$cmd" == "start" ]]; then
    for i in 1 2 3; do
        ...
        export HADOOP_HOME=$basedir/target/dfsnode$i
        screen -c /dev/null -dmS journalnode$i -L -Logfile $basedir/target/journalnode$i.log $HADOOP_HOME/bin/hdfs --config $HADOOP_HOME/etc/hadoop journalnode
    done
    for i in 1 2 3; do
    ...
    if ! isAddrUp 127.0.0.1:8020 ; then
	    ...
	    screen -c /dev/null -dmS namenode1 -L -Logfile $basedir/target/namenode1.log $HADOOP_HOME/bin/hdfs --config $HADOOP_HOME/etc/hadoop namenode
    ...
    if ! isAddrUp 127.0.0.1:8021 ; then
	    ...
	    screen -c /dev/null -dmS namenode2 -L -Logfile $basedir/target/namenode2.log $HADOOP_HOME/bin/hdfs --config $HADOOP_HOME/etc/hadoop namenode
    ...
    for i in 1 2 3; do
        ...
        export HADOOP_HOME=$basedir/target/dfsnode$i
        screen -c /dev/null -dmS datanode$i -L -Logfile $basedir/target/datanode$i.log $HADOOP_HOME/bin/hdfs --config $HADOOP_HOME/etc/hadoop datanode
    done
    for i in 1 2 3; do
